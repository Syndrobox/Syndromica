...
api.constants = {
  // CURRENCY_CODE: 'USD',
  // SELLER_NOTE: 'syndromica Payment',
  // SELLER_NOTE_SUBSCRIPTION: 'syndromica Subscription',
  // SELLER_NOTE_ATHORIZATION_SUBSCRIPTION: 'syndromica Subscription Payment',
  // STORE_NAME: 'syndromica',
  //
  // GIFT_TYPE_GEMS: 'gems',
...
api.checkout = async function checkout (options = {}) {
  ...
  let amount;
  let gemsBlock;
  let description = 'syndromica Gems';
  ...
  if (gift) {
    ...
    if (gift.type === 'gems') {
      ...
      description = 'mo. syndromica Subscription (Gift)';
    }
  } else if (sku) {
...
api.subscribe = async function subscribe (options = {}) {
  ...
  const billingPlanTitle = `syndromica Subscription ($${sub.price} every ${sub.months} months, recurring)`;
