...
export async function createChallenge (user, req, res) {
  ...
  categories.forEach(category => {
    if (category.slug === 'Syndromica_official' && !user.hasPermission('challengeAdmin')) {
      throw new NotAuthorized(res.t('noPrivAccess'));
    } else if (category.slug === 'Syndromica_official' && user.hasPermission('challengeAdmin')) {
            ...
            $gt: [
              {
                $size: {
                  $filter: {
                    ...
                    as: 'cat',
                    cond: {
                      $eq: ['$$cat.slug', 'Syndromica_official'],
                    },
                  },
                },
              },
