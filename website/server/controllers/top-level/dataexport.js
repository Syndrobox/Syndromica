...
// import got from 'got';
import md from 'syndromica-markdown';
 ...
 * @apiDescription History is only available for habits and dailies so todos and rewards won't be included. Can only be used on [https://syndromica.com](https://syndromica.com).
...
api.exportUserHistory = {
  ...
  async handler (req, res) {
    ...
    res.set({
      ...
      'Content-disposition': 'attachment; filename=syndromica-tasks-history.csv',
    });
...
api.exportUserDataJson = {
  ...
  async handler (req, res) {
    ...
    res.set({
      ...
      'Content-disposition': 'attachment; filename=syndromica-user-data.json',
    });
 ...
 * @apiDescription This XML export feature is not currently working (https://github.com/Syndrobox/syndromica/issues/10100).
 * @apiGroup DataExport
 *
...
api.exportUserDataXml = {
  ...
  async handler (req, res) {
    ...
    res.set({
      ...
      'Content-disposition': 'attachment; filename=syndromica-user-data.xml',
    });
 ...
 * @apiDescription This HTML export feature is not currently working (https://github.com/Syndrobox/syndromica/issues/9489).
 * @apiGroup DataExport
 *
...
api.exportUserAvatarHtml = {
  ...
  // middlewares: [locals],
  async handler (/* req, res */) {
    throw new NotFound('This API route is currently not available. See https://github.com/Syndrobox/syndromica/issues/9489.');
    ...
    res.render('avatar-static', {
      ...
      env: _.defaults({ user: member }, res.locals.syndrobox),
    }); */
  },
};
 ...
 * @apiDescription This PNG export feature is not currently working (https://github.com/Syndrobox/syndromica/issues/9489).
 * @apiGroup DataExport
 *
 * @apiParam (Path) {String} uuid The User ID of the user
 *
