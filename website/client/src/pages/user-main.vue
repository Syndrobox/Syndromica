...
export default {
  ...
  created () {
    this.$root.$on('playSound', sound => {
      ...
      const file = `https://syndromica-assets.s3.amazonaws.com/mobileApp/sounds/${theme}/${sound}`;
    ...
    Promise.all([
      ...
      if (window && window['syndromica-i18n']) {
        if (this.user.preferences.language === window['syndromica-i18n'].language.code) {
          return null;
        }
      }
      if (window && window['syndromica-i18n']) {
      ...
      const i18nData = window && window['syndromica-i18n'];
      ...
      if (appState) {
        ...
        if (appState.paymentCompleted) {
          ...
          this.$root.$emit('syndromica:payment-success', appState);
        }
      }
      this.$nextTick(() => {
