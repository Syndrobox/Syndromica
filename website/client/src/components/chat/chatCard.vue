...
export default {
  ...
  methods: {
    ...
    copyAsTodo (message) {
      this.$root.$emit('syndromica::copy-as-todo', message);
    },
    report () {
      this.$root.$on('syndromica:report-result', data => {
        if (data.ok) {
          this.reported = true;
        }

        this.$root.$off('syndromica:report-result');
      });

      this.$root.$emit('syndromica::report-chat', {
        message: this.msg,
