...
export default {
  ...
  mounted () {
    this.$root.$on('syndromica::report-chat', this.handleReport);
  },
  beforeDestroy () {
    this.$root.$off('syndromica::report-chat', this.handleReport);
  },
  methods: {
    close () {
    ...
    async reportAbuse () {
      ...
      this.$root.$emit('syndromica:report-result', result);
