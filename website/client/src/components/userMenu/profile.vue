...
const COMMUNITY_MANAGER_EMAIL = 'admin@syndromica.com';
...
export default {
  ...
  mounted () {
    ...
    this.$root.$on('syndromica:report-profile-result', () => {
      this.loadUser();
    });
  },
  updated () {
  ...
  beforeDestroy () {
    ...
    this.$root.$off('syndromica:report-profile-result');
  },
  methods: {
    async loadUser () {
      let user = null;
    ...
    openSendGemsModal () {
      ...
      this.$root.$emit('syndromica::send-gift', this.user);
    },
    ...
    reportPlayer () {
      this.$root.$emit('syndromica::report-profile', {
