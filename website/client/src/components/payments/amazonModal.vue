...
const SyndromicaUrl = `${window.location.protocol}//${window.location.host}`;
...
export default {
  ...
  mounted () {
    this.$root.$on('syndromica::pay-with-amazon', amazonPaymentsData => {
  ...
  beforeDestroy () {
    this.$root.$off('syndromica::pay-with-amazon');
  },
  methods: {
    amazonInitWidgets () {
      const walletParams = {
        ...
        try {
          ...
          if (newGroup && newGroup._id) {
            ...
            if (!this.$store.state.isUserLoggedIn) {
              this.storePaymentStatusAndReload(`${SyndromicaUrl}/group-plans/${newGroup._id}/task-information?showGroupOverview=true`);
              return;
            }

            this.user.guilds.push(newGroup._id);
            this.storePaymentStatusAndReload(`${SyndromicaUrl}/group-plans/${newGroup._id}/task-information`);
            return;
          }

          if (this.amazonPayments.groupId) {
            this.storePaymentStatusAndReload(`${SyndromicaUrl}/group-plans/${this.amazonPayments.groupId}/task-information`);
            return;
          }
