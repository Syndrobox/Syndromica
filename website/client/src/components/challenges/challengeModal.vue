      ...
      >
        ...
        >
          ...
          <div
            ...
            v-if="group.key !== 'Syndromica_official' || hasPermission(user, 'challengeAdmin')"
            :key="group.key"
            class="form-check"
          >
  ...
  strong(v-once) {{$t('endDate')}}
      ...
      <div class="row footer-wrap">
        <div class="col-12 text-center submit-button-wrapper">
          ...
          <!-- see https://github.com/Syndrobox/syndromica/blob/develop/website/views/options/social/challenges.jade#L134-->
          <button
...
export default {
  ...
  mounted () {
    this.$root.$on('syndromica:clone-challenge', data => {
    ...
    this.$root.$on('syndromica:update-challenge', data => {
      if (!data.challenge) return;
      this.cloning = false;
      this.$store.state.challengeOptions.workingChallenge = {
        ...this.$store.state.challengeOptions.workingChallenge,
    ...
    this.$root.$on('syndromica:create-challenge', () => {
      this.cloning = false;
      this.$store.state.challengeOptions.workingChallenge = {};
      this.$root.$emit('bv::show::modal', 'challenge-modal');
    });
  ...
  beforeDestroy () {
    this.$root.$off('syndromica:clone-challenge');
    this.$root.$off('syndromica:update-challenge');
    this.$root.$off('syndromica:create-challenge');
  },
  methods: {
    async shown () {
