...
export default {
  ...
  computed: {
    ...
    challengeName () {
      ...
      return SyndromicaMarkdown.render(String(this.notification.data.name));
    },
  },
  mounted () {
    this.$root.$on('syndromica:won-challenge', notification => {
      this.notification = notification;
      this.$root.$emit('bv::show::modal', 'won-challenge');
    });
  },
  beforeDestroy () {
    this.$root.$off('syndromica:won-challenge');
  },
  methods: {
    close () {
