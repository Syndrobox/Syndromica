...
export default {
  ...
  mounted () {
    this.$root.$on('syndromica::report-profile', this.handleReport);
  },
  beforeDestroy () {
    this.$root.$off('syndromica::report-profile', this.handleReport);
  },
  methods: {
    close () {
    ...
    async reportAbuse () {
      ...
      if (result.status === 200) {
        ...
        this.$root.$emit('syndromica:report-profile-result', result.data.data);
      } else {
