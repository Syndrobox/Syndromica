...
async function deleteSyndromicaData (user, email) {
  const set = {
    'auth.blocked': false,
  ...
  if (response) {
    if (response.status === 200) {
      console.log(`${user._id} (${email}) removed from syndromica. Last login: ${user.auth.timestamps.loggedin}`);
    } else {
      console.log(`${user._id} (${email}) syndromica response: ${response.status} ${response.statusText}`);
    }
  }
}
...
async function processEmailAddress (email) {
  ...
  return Promise.all(users.map(user => (async () => {
    ...
    await deleteSyndromicaData(user, email); // eslint-disable-line no-await-in-loop
  })()));
}
