...
describe('redirects middleware', () => {
  ...
  context('forceSSL', () => {
    it('sends http requests to https', () => {
      ...
      nconfStub.withArgs('BASE_URL').returns('https://syndromica.com');
      ...
      expect(res.redirect).to.be.calledWith('https://syndromica.com/static/front');
    });
    ...
    it('does not redirect if base URL is not https', () => {
      const nconfStub = sandbox.stub(nconf, 'get');
      nconfStub.withArgs('BASE_URL').returns('http://syndromica.com');
      nconfStub.withArgs('IS_PROD').returns(true);
      req.protocol = 'http';
      req.originalUrl = '/static/front';
    ...
    it('does redirect if skip ssl request param is passed with incorrrect key', () => {
      ...
      expect(res.redirect).to.be.calledOnce;
      expect(res.redirect).to.be.calledWith('https://syndromica.com/static/front?skipSSLCheck=INVALID');
    });
  ...
  context('forceSyndromica', () => {
    it('sends requests with differing hostname to base URL host', () => {
      ...
      attachRedirects.forceSyndromica(req, res, next);

      expect(res.redirect).to.be.calledOnce;
      expect(res.redirect).to.be.calledWith(301, 'https://syndromica.com/static/front');
    });
