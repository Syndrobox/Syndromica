...
describe('paypal - checkout', () => {
  ...
  it('creates a link for gem purchases', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('syndromica Gems', 4.99));
  ...
  it('creates a link for gifting gems', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledOnce;
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('syndromica Gems (Gift)', '4.00'));
    expect(link).to.eql(approvalHerf);
  });
  ...
  it('creates a link for gifting a subscription', async () => {
    ...
    expect(paypalPaymentCreateStub).to.be.calledOnce;
    expect(paypalPaymentCreateStub).to.be.calledWith(getPaypalCreateOptions('mo. syndromica Subscription (Gift)', '15.00'));
    expect(link).to.eql(approvalHerf);
  });
});
