...
describe('DELETE /user', () => {
  let user;
  const password = 'password'; // from syndrobox/test/helpers/api-integration/v3/object-generators.js
  ...
  context('user with local auth', async () => {
    ...
    it('returns an error if excessive feedback is supplied', async () => {
      ...
      await expect(user.del('/user', {
        ...
        code: 400,
        error: 'BadRequest',
        message: 'Account deletion feedback is limited to 10,000 characters. For lengthy feedback, email admin@syndromica.com.',
      });
    });
